...
class TextMsg(BaseMsg):
    ...
    role: Literal["user", "assistant", "role1", "role2"]
    """消息来源"""
    content: str
    """消息内容"""
...
def build_history_msg(role: str, messages: MsgList) -> TextMsgList:
    history_messages = []
    if messages is None:
    ...
    for msg in messages:
        if msg["role"] != "image":
            if msg["role"] == role:
                history_messages.append(TextMsg(role="assistant", content=msg["content"]))
            else:
                history_messages.append(TextMsg(role="user", content=msg["content"]))
    return history_messages
