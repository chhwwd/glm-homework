...
# 风格选项
options = ['传统', '复古', '卡通', '二次元', '艺术', '超现实']

# 2x2 layout
with st.container():
    col1, col2 = st.columns(2)
    with col1:
        st.text_input(label="角色名", value="", key="bot_name", on_change=lambda: st.session_state["meta"].update(bot_name=st.session_state["bot_name"]), help="模型所扮演的角色的名字，不可以为空")
        st.text_area(label="角色人设", value="", key="bot_info", on_change=lambda: st.session_state["meta"].update(bot_info=st.session_state["bot_info"]), help="角色的详细人设信息，不可以为空")
        st.selectbox(label="风格", options=options, key="style", on_change=lambda: st.session_state["meta"].update(style=st.session_state["style"]), help="风格")
